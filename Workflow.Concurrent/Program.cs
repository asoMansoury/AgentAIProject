...
AIAgent legalAgent = chatClient.AsAIAgent(name: "LegalAgent", instructions: "You are a legal agent that need to evaluate if a text is legal(use max 200 chars)");
AIAgent spellingErrorAgent = chatClient.AsAIAgent(name: "SpellingErrorAgent", instructions: "ou are a spelling expert(use max 200 chars");
...
var messages = new List<Microsoft.Extensions.AI.ChatMessage> { new(ChatRole.User, legalText) };
StreamingRun run = await InProcessExecution.RunStreamingAsync(workflow, messages);
await run.TrySendMessageAsync(new TurnToken(emitEvents: true));

List<Microsoft.Extensions.AI.ChatMessage> responseMessages = new();

await foreach (WorkflowEvent evt in run.WatchStreamAsync().ConfigureAwait(false))
{
    if (evt is WorkflowOutputEvent completed)
    {
        responseMessages = (List<Microsoft.Extensions.AI.ChatMessage>)completed.Data!;
        break;
    }
}
