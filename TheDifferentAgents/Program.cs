...
//The Raw client(OpenAiClient and AzureOpenAIClient wors that same)
...
//IChatClient is Microsoft.Extensions.AI's abstraction layer, you can use it with Azure AI Foundry, Azure OpenAI and OpenAI
//This is what other non-Open AI providers can implement to be compatible with Microsoft.Extensions.AI's features like AIAgent
...
PersistentAgent persistentAgentForAIAgent = (await persistentAgentsClient.Administration.CreateAgentAsync(model)).Value;
AIAgent aiAgentFromPersistentClient = new ChatClientAgent(
    rawOpenAiClient.GetChatClient(persistentAgentForAIAgent.Model).AsIChatClient()
);
AIAgent aiAgentFromIChatClient = new ChatClientAgent(iChatClient);
//-----------------------------------------------------------------

//Most is however a ChatClientAgent behine the scenes
...
#pragma warning restore OPENAI001 // Type is for evaluation purposes only and is subject to change or removal in future updates. Suppress this diagnostic to proceed.
PersistentAgent persistentAgent = (await persistentAgentsClient.Administration.CreateAgentAsync(model)).Value;
ChatClientAgent chatClientAgentFromPersistentClient = new ChatClientAgent(
    rawOpenAiClient.GetChatClient(persistentAgent.Model).AsIChatClient()
);
...
//Middlware Builder can only produce an AI Agent :-(
